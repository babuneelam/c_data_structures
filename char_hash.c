...
typedef struct hash_node {	
    ...
    char key; // Keep towards the end for alignment
} hash_node_t;
...
char help_str[] = "Allowed Operations: \n"
    ...
    "    2 <char key> <val>- Insert ele\r\n"
    "    3 <char key> - Delete ele\r\n"
    "    4 <char key> - search ele\r\n"
...
int main()
{
	...
	switch(op)
	{
		...
		ret = ht_delete(key);
		if (!ret)
		    printf("key %c not found in hash table\r\n", key);
		break;
	    case 4: /* search */
		getchar();
		scanf(" %c", &key);
...
static __always_inline unsigned int hash_fn(char key)
{
    ...
    // just returning key. This simple hash algo is sufficient as the 
    // ASCII char space is smaller: 0-127
    return(key % MAX_HASH); 
}
...
void ht_display()
{
	...
	while(h)
	{
	    printf("key %c val %d, ", h->key, h->val);
	    h=h->next;
	}
	printf("\r\n");
    }
